...
TEST( graphMST , MSTKruskal )
{
  ...
  graph_type mst = MST_solver.MSTKruskal( g ) ;
...
TEST( graphMST , MSTPrim )
{
  ...
  graph_type mst = MST_solver.MSTPrim( g ) ;
  ...
  std::ofstream file2( "mst_prim.dot" );
  file2 << g_viz_mst << std::endl ;
  file2.close() ;
}

TEST( graphMST , EdgeMST_SpecialWeightsKruskal )
{
  ...
  graph_type mst = MST_solver.MSTKruskal( g , edge_w ) ;
...
TEST( graphMST , EdgeMST_SpecialWeightsPrim )
{
  typedef UndirectedGraph<std::string, std::string> graph_type ;
  graph_type g ;
  ...
  graph_type mst = MST_solver.MSTPrim( g , edge_w ) ;
  ...
  std::ofstream file2( "mst_w_prim.dot" );
  file2 << g_viz_mst << std::endl ;
  file2.close() ;
}
