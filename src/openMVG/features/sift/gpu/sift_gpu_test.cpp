...
TEST( GaussianScaleSpaceGPU , OctaveGeneration )
{
  ...
  HierarchicalGaussianScaleSpaceGPU octave_gen( 6, 3, GaussianScaleSpaceParams( 1.6f, 1.0f, 0.5f ) , ctx );
  ...
  while ( octave_gen.NextOctave( gpu_octave ) )
  {
    ...
    for ( int i = 0; i < cpu_octave.slices.size(); ++i )
    {
      std::stringstream str;
      str << "gaussian_octave_" << std::to_string( octave_id ) << "_" << i << ".png";
      EXPECT_TRUE( WriteImage( str.str().c_str() , Image<unsigned char>( cpu_octave.slices[i].cast<unsigned char>()  ) ) );
    }
    ++octave_id;
  }
}

TEST( Sift_KeypointGPU , DetectionAndDescription )
{
  Image<unsigned char> in;
  ...
  HierarchicalGaussianScaleSpaceGPU octave_gen( 6, 3, GaussianScaleSpaceParams( 1.6f, 1.0f, 0.5f, supplementary_images ) , ctx );
  GPUOctave gpu_octave ;
  Octave cpu_octave ;
...
TEST( Sift , EmptyImage )
{
  ...
  SIFT_Anatomy_Image_describerGPU extractor;
