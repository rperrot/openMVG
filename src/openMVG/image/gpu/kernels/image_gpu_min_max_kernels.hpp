...
const std::string krnsImageLocalMaxima =
  R"(
      ...
      // output is 1.f if pixel is a local max 0.f if not
      // Note local max search is in B
      __kernel void image_local_min_max_range_3d( write_only image2d_t res , 
                                                  read_only image2d_t imgA , 
                                                  read_only image2d_t imgB ,
                                                  read_only image2d_t imgC ,
                                                  ...
                                                  const int2 region_size )
      {
        ...
        // Center position of the kernel 
        const int pix_x = group_id_x * WORK_GROUP_SIZE + local_x ;
        const int pix_y = group_id_y * WORK_GROUP_SIZE + local_y ;
        ...
        if( pix_x < max_x && pix_y < max_y && pix_x >= min_x && pix_y >= min_y ) 
        {
          const float4 pix = cachedData[1][ local_x + 1 ][ local_y + 1 ] ; 

          const int4 local_min_max = 
            // Bellow 
            cachedData[0][ local_x ][ local_y ]         < pix && 
            cachedData[0][ local_x + 1 ][ local_y ]     < pix && 
            cachedData[0][ local_x + 2 ][ local_y ]     < pix &&
      ...
      // output is 1.f if pixel is a local max 0.f if not
      // Note local max search is in B
      __kernel void image_local_min_max_range_3d_threshold( write_only image2d_t res , 
                                                  read_only image2d_t imgA , 
                                                  read_only image2d_t imgB ,
                                                  read_only image2d_t imgC ,
                                                  const int2 offset_region , 
