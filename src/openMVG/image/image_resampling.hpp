...
namespace image
{
  ...
  /**
   * @brief Rescale image by a scaling factor using linear sampling
   * @param src Input image 
   * @param out Output image 
   * @param scale Scaling factor between src and out 
   */
  template <typename Image>
  void ImageRescale( const Image &src, Image &out, const double scale )
  {
    const int new_width  = src.Width() * scale;
    const int new_height = src.Height() * scale;
    ...
    for( int id_row = 0 ; id_row < new_height ; ++id_row )
    {
      for( int id_col = 0 ; id_col < new_width ; ++id_col )
      {
        const int old_row = std::min( (int) (id_row * inv_scale) , (int) (src.Width() - 1) ) ; 
        const int old_col = std::min( (int) (id_col * inv_scale) , (int) (src.Height() - 1) ) ; 

        out( id_row , id_col ) = sampler( src , old_row , old_col ) ;  
      }
    }
  }

  /**
  * @brief Half sample an image (ie reduce it's size by a factor 2) using bilinear interpolation
  * @param src input image
  * @param out output image
  */
  template <typename Image>
