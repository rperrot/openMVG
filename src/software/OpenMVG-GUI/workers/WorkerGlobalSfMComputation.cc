...
*/
WorkerGlobalSfMComputation::WorkerGlobalSfMComputation( std::shared_ptr<Project> project ,
...
void WorkerGlobalSfMComputation::process( void )
{
  ...
  openMVG::sfm::GlobalSfMReconstructionEngine_RelativeMotions sfmEngine(
    *sfm_data,
    globalPath,
    stlplus::create_filespec( globalPath, "Reconstruction_Report.html" ) );
  ...
  */
  ...
  if ( sfmEngine.Process() )
  {
    ...
    Generate_SfM_Report( sfmEngine.Get_SfM_Data(),
                         stlplus::create_filespec( globalPath, "SfMReconstruction_Report.html" ) );

    //-- Export to disk computed scene (data & visualizable results)
    std::cout << "...Export SfM_Data to disk." << std::endl;
    Save( sfmEngine.Get_SfM_Data(),
          stlplus::create_filespec( globalPath, "sfm_data", ".bin" ),
          openMVG::sfm::ESfM_Data( openMVG::sfm::ESfM_Data::ALL ) );

    Save( sfmEngine.Get_SfM_Data(),
          stlplus::create_filespec( globalPath, "cloud_and_poses", ".ply" ),
          openMVG::sfm::ESfM_Data( openMVG::sfm::ESfM_Data::ALL ) );
