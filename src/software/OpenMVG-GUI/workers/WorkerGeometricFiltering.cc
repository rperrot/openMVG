...
WorkerGeometricFiltering::WorkerGeometricFiltering( std::shared_ptr<Project> &project, std::shared_ptr<openMVG::sfm::Regions_Provider> &regions_provider, std::shared_ptr<openMVG::matching::PairWiseMatches> &map_putative, const WorkerNextAction &na )
  : WorkerInterface( na )
  , m_project( project )
  ...
  if ( filter_ptr )
  {
    ...
    switch ( m_params.geometricModel() )
    {
      case MATCHING_GEOMETRY_TYPE_HOMOGRAPHY:
      {
        ...
        filter_ptr->Robust_model_estimation( GeometricFilter_HMatrix_AC( 4.0, imax_iteration ), *m_map_putative, bGuided_matching, bGeometric_only_guided_matching ? -1.0 : d_distance_ratio, progressInterface );
      ...
      {
        filter_ptr->Robust_model_estimation( GeometricFilter_FMatrix_AC( 4.0, imax_iteration ), *m_map_putative, bGuided_matching, d_distance_ratio, progressInterface );
        m_map_filtered            = std::make_shared<openMVG::matching::PairWiseMatches>( filter_ptr->Get_geometric_matches() );
      ...
      {
        filter_ptr->Robust_model_estimation( GeometricFilter_EMatrix_AC( 4.0, imax_iteration ), *m_map_putative, bGuided_matching, d_distance_ratio, progressInterface );
        m_map_filtered = std::make_shared<openMVG::matching::PairWiseMatches>( filter_ptr->Get_geometric_matches() );

        //-- Perform an additional check to remove pairs with poor overlap
        std::vector<openMVG::matching::PairWiseMatches::key_type> vec_toRemove;
        for ( openMVG::matching::PairWiseMatches::const_iterator iterMap = m_map_filtered->begin(); iterMap != m_map_filtered->end(); ++iterMap )
        {
        ...
        for ( std::vector<openMVG::matching::PairWiseMatches::key_type>::const_iterator iter = vec_toRemove.begin(); iter != vec_toRemove.end(); ++iter )
        {
    ...
    //---------------------------------------
    if ( !Save( *m_map_filtered, stlplus::create_filespec( stlplus::folder_append_separator( sFeaturePath ), sGeometricMatchesFilename ) ) )
    {
      // std::cerr
