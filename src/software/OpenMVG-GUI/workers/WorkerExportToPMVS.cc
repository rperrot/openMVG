...
WorkerExportToPMVS::WorkerExportToPMVS( std::shared_ptr<openMVG::sfm::SfM_Data> sfm_data, const std::string &output_directory, const int downsampling_factor, const int cpu_count, const bool export_vis )
  : m_sfm_data( sfm_data )
  ...
  else
  {
    ...
    for ( Views::const_iterator iter = sfm_data->GetViews().begin(); iter != sfm_data->GetViews().end(); ++iter )
    {
      ...
      if ( isPinhole( iterIntrinsic->second->getType() ) )
      {
        ...
        osList << stlplus::basename_part( view->s_Img_path ) + "." + stlplus::extension_part( view->s_Img_path ) << " 0 " << focal << os.widen( '\n' );
      }
      else
      {
...
void WorkerExportToPMVS::process( void )
{
  ...
  if ( !( stlplus::is_folder( stlplus::folder_append_separator( m_output_directory ) + "models" ) && stlplus::is_folder( stlplus::folder_append_separator( m_output_directory ) + "txt" ) && stlplus::is_folder( stlplus::folder_append_separator( m_output_directory ) + "visualize" ) ) )
  {
  ...
  for ( Views::const_iterator iter = m_sfm_data->GetViews().begin(); iter != m_sfm_data->GetViews().end(); ++iter, hasIncremented( 1 ) )
  {
    const View *view = iter->second.get();
    ...
    std::ofstream file( stlplus::create_filespec( stlplus::folder_append_separator( m_output_directory ) + "txt", os.str(), "txt" ).c_str() );
  ...
  for ( int i = 0; i < static_cast<int>( views.size() ); ++i )
  {
    ...
    std::ostringstream os;
    os << std::setw( 8 ) << std::setfill( '0' ) << map_viewIdToContiguous[ view->id_view ];
    const std::string dstImage = stlplus::create_filespec( stlplus::folder_append_separator( m_output_directory ) + "visualize", os.str(), "jpg" );
    ...
    else // (no distortion)
    {
      ...
      if ( stlplus::extension_part( srcImage ) == "JPG" || stlplus::extension_part( srcImage ) == "jpg" )
      {
  ...
  if ( m_export_vis_file )
  {
    ...
    for ( Landmarks::const_iterator itL = m_sfm_data->GetLandmarks().begin(); itL != m_sfm_data->GetLandmarks().end(); ++itL )
    {
  ...
  // Finally
  exportToBundlerFormat( m_sfm_data, stlplus::folder_append_separator( m_output_directory ) + stlplus::folder_append_separator( "PMVS" ) + "bundle.rd.out", stlplus::folder_append_separator( m_output_directory ) + stlplus::folder_append_separator( "PMVS" ) + "list.txt" );
