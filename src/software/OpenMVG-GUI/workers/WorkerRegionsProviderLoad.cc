 ...
 * @brief ctr
 */
WorkerRegionsProviderLoad::WorkerRegionsProviderLoad( std::shared_ptr<Project> &pro, const WorkerNextAction &na )
  : WorkerInterface( na )
  , m_project( pro )
{
}
...
void WorkerRegionsProviderLoad::process( void )
{
  ...
  std::unique_ptr<openMVG::features::Regions> regions_type =
    openMVG::features::Init_region_type_from_file( sImage_describer );
  if ( !regions_type )
  {
