 ...
 */
SfMDataHelper::SfMDataHelper( std::shared_ptr<openMVG::sfm::SfM_Data> sfm_data ) : m_sfm_data( sfm_data )
{
}
...
std::vector<std::pair<int, double>> SfMDataHelper::linkedViewsWithStrength( const int id ) const
{
  ...
  std::vector<uint32_t> nbOccurrence;
  {
    ...
    nbOccurrence.resize( max_view_id + 1 , static_cast<uint32_t>( 0 ) );
  }
  ...
  for ( const auto &it_landmark : m_sfm_data->GetLandmarks() )
  {
    ...
    for ( const auto &it_obs : obs )
    {
      views_for_this_track.push_back( it_obs.first );
      if ( id == it_obs.first )
      {
        track_interesting = true;
      }
    ...
    if ( track_interesting )
    {
      for ( const auto &it_view : views_for_this_track )
      {
        if ( it_view != id )
        {
          ++nbOccurrence[ it_view ];
          connected_views.push_back( it_view );
        }
      }
    }
  }
  ...
  const int nb = std::distance( connected_views.begin(), end );
  if ( nb == 0 )
  {
    return {};
  }
  std::vector<std::pair<int, double>> result( nb );
