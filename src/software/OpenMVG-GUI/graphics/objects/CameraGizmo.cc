 ...
 */
CameraGizmo::CameraGizmo( std::shared_ptr<OpenGLContext> ctx, std::shared_ptr<ShaderProgram> shad,
                          const openMVG::geometry::Pose3 &pose, const bool is_spherical, const double size ) :
    RenderableObject( ctx, shad ),
    m_pose( pose ), m_size( size ), m_is_spherical( is_spherical ), m_nb_vert( 0 )
{
}
...
void CameraGizmo::prepare( void )
{
  ...
  if ( !m_is_spherical )
  {
    ...
    if ( col != -1 )
    {
      ...
      glFuncs->glVertexAttribPointer( col, 3, GL_FLOAT, GL_FALSE, 6 * sizeof( GLfloat ),
      ...
      for ( int id_theta = 0; id_theta < nb_theta_segment; ++id_theta )
      {
        ...
        */
        const openMVG::Vec3 A( 0.5 * std::sin( phi_cur ) * std::cos( theta_cur ),
                               0.5 * std::sin( phi_cur ) * std::sin( theta_cur ), 0.5 * std::cos( phi_cur ) );
        const openMVG::Vec3 B( 0.5 * std::sin( phi_cur ) * std::cos( theta_next ),
                               0.5 * std::sin( phi_cur ) * std::sin( theta_next ), 0.5 * std::cos( phi_cur ) );
        const openMVG::Vec3 C( 0.5 * std::sin( phi_next ) * std::cos( theta_cur ),
                               0.5 * std::sin( phi_next ) * std::sin( theta_cur ), 0.5 * std::cos( phi_next ) );
        const openMVG::Vec3 D( 0.5 * std::sin( phi_next ) * std::cos( theta_next ),
                               0.5 * std::sin( phi_next ) * std::sin( theta_next ), 0.5 * std::cos( phi_next ) );
  ...
  if ( selected() )
  {
    ...
    else
    {
      ...
      */
      ...
      m_shader->setUniform( "uColor", col );
    }
 ...
 */
Intersection CameraGizmo::intersect( const Ray &ray ) const
{
  if ( m_is_spherical )
  {
  ...
  {
    const openMVG::Vec3 C  = m_pose.center();
    const openMVG::Mat3 Rt = m_pose.rotation().inverse();

    // Vertices of the gizmo
    const openMVG::Vec3 v1 = C;
    const openMVG::Vec3 v2 = m_size * Rt * ( ( openMVG::Vec3( -0.5, -0.5, 1.0 ) ) ) + C;
    const openMVG::Vec3 v3 = m_size * Rt * ( ( openMVG::Vec3( -0.5, 0.5, 1.0 ) ) ) + C;
    const openMVG::Vec3 v4 = m_size * Rt * ( ( openMVG::Vec3( 0.5, 0.5, 1.0 ) ) ) + C;
    const openMVG::Vec3 v5 = m_size * Rt * ( ( openMVG::Vec3( 0.5, -0.5, 1.0 ) ) ) + C;

    // Intersect with the bounding box of the gizmo (maybe not tight as expected but simple enough)
    AABB box;
    box = makeUnion( box, v1 );
    box = makeUnion( box, v2 );
    box = makeUnion( box, v3 );
    box = makeUnion( box, v4 );
    box = makeUnion( box, v5 );

    Intersection inter = box.intersect( ray );
    if ( inter )
    {
      return Intersection( true, std::const_pointer_cast<IntersectableObject>( shared_from_this() ), inter.distance() );
    }
    return Intersection();
  }
}
