...
void ResultViewWidget::initializeGL( void )
{
  ...
  // Create point shader
  const std::string ressource_path = stlplus::folder_append_separator( QCoreApplication::applicationDirPath().toStdString() ) + "ressources" ;
...
void ResultViewWidget::mouseMoveEvent( QMouseEvent * event )
{
  ...
  switch( cur_act )
  {
    ...
    case ROTATE :
    {
      ...
      const openMVG::Vec3 realOld = ball.get( m_last_mouse_x - width() / 2.0 , ( static_cast<double>( height() ) - m_last_mouse_y ) - height() / 2.0 ) ;
      const openMVG::Vec3 realNew = ball.get( event->x() - width() / 2.0 , ( static_cast<double>( height() ) - event->y() ) - height() / 2.0 ) ;
