                  ...
                  cl_kernel                                                   sort_and_store_cost_kernel )
{
  ...
  cl_mem planes_n = wrapper.createBuffer( nb_pixel * 3 * sizeof( cl_float ),
                                          OpenCLWrapper::OPENCL_BUFFER_READ_ONLY,
                                          tmp_planes_n );
  ...
  ComputeCost( outCost, reference_cam, neigh_imgs, cams, stereo_rig, params, -1, Ip, Gp, CensusP, Kinv, planes_n, planes_d, wrapper, cost_kernel, cost_kernel, cost_kernel, append_cost_kernel, sort_and_store_cost_kernel, compute_size );
                  ...
                  cl_kernel                                                   sort_and_store_cost_kernel )
{
  ...
  // Now compute cost
  cl_mem outCost = wrapper.createBuffer( nb_pixel * sizeof( cl_float ), OpenCLWrapper::OPENCL_BUFFER_READ_WRITE );
  ComputeCost( outCost, reference_cam, neigh_imgs, cams, stereo_rig, params, scale, Ip, Gp, CensusP, Kinv, planes_n, planes_d, wrapper, cost_kernel, cost_kernel, cost_kernel, append_cost_kernel, sort_and_store_cost_kernel, compute_size );
...
*/
void Propagate( DepthMap& map, const int id_start, Camera& reference_cam, const std::vector<Camera>& cams, const std::vector<std::pair<openMVG::Mat3, openMVG::Vec3>>& stereo_rig, const Image& image_ref, const DepthMapComputationParameters& params, OpenCLWrapper& wrapper,
                ...
                cl_kernel& compute_pixel_depth_kernel )  // Compute pixel depth value
{
  ...
  for ( int id_plane = 0; id_plane < 20; ++id_plane )
  {
    ...
    ComputeCost( cur_cost, reference_cam, neigh_imgs, cams, stereo_rig, params, -1, Ip, Gp, CensusP, Kinv, best_plane_n, best_plane_d, wrapper, kernel_red /* unused */, kernel_red, kernel_black, append_cost_kernel, sort_and_store_cost_kernel, compute_size, delta_plane, c_type, &all_costs );
...
*/
void Propagate( DepthMap& map, const int id_start, Camera& reference_cam, const std::vector<Camera>& cams, const std::vector<std::pair<openMVG::Mat3, openMVG::Vec3>>& stereo_rig, const Image& image_ref, const std::vector<Image>& neigh_imgs, const DepthMapComputationParameters& params, const int scale, OpenCLWrapper& wrapper,
                cl_kernel& kernel_red,                   // Compute cost
                cl_kernel& kernel_black,                 // Compute cost
                cl_kernel& append_cost_kernel,           // Append cost
                cl_kernel& sort_and_store_cost_kernel,   // Sort and compute real cost
                ...
                cl_kernel& compute_pixel_depth_kernel )  // Compute pixel depth value
{
  ...
  for ( int id_plane = 0; id_plane < 20; ++id_plane )
  {
    ...
    ComputeCost( cur_cost, reference_cam, neigh_imgs, cams, stereo_rig, params, scale, Ip, Gp, CensusP, Kinv, best_plane_n, best_plane_d, wrapper, kernel_red /* unused */, kernel_red, kernel_black, append_cost_kernel, sort_and_store_cost_kernel, compute_size, delta_plane, c_type, &all_costs );
                 ...
                 cl_kernel&                                                  compute_new_plane_kernel )                                                      // Compute new planes
{
  ...
  const double   min_disparity   = cam.depthDisparityConversion( cam.m_max_depth, params.scale() );
  const double   max_disparity   = cam.depthDisparityConversion( cam.m_min_depth, params.scale() );
  ...
  while ( delta_disparity > 0.01f )
  {
    ...
    // Compute cost
    ComputeCost( tmp_cost, cam, neigh_imgs, cams, stereo_rig, params, -1, Ip, Gp, CensusP, Kinv, out_plane_n, out_plane_d, wrapper, kernel_full, kernel_full, kernel_full, append_cost_kernel, sort_and_store_cost_kernel, compute_size, delta_plane, c_type, &all_costs );
                 ...
                 cl_kernel&                                                  compute_new_plane_kernel )                                                      // Compute new planes
{
  ...
  while ( delta_disparity > 0.01f )
  {
    ...
    // Compute cost
    ComputeCost( tmp_cost, cam, neigh_imgs, cams, stereo_rig, params, scale, Ip, Gp, CensusP, Kinv, out_plane_n, out_plane_d, wrapper, kernel_full, kernel_full, kernel_full, append_cost_kernel, sort_and_store_cost_kernel, compute_size, delta_plane, c_type, &all_costs );
